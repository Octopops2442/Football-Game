...
let secondPerson = new PointerLockControls(
  overHeadCamera2,
  renderer.domElement
);
...
function animate() {
  ...
  if (human1Camera || human2Camera) {
    // either human 1 or human 2 over head camera
  ...
  tree2.position.set(100, 0, -60);
  tree2Light.target = tree2;
  tree2Light.position.set(tree2.position.x, 30, tree2.position.z);

  housePlant.position.set(-100, 0, 60);
  plantLight.target = housePlant;
  plantLight.position.set(housePlant.position.x, 30, housePlant.position.z);

  housePlant2.position.set(-100, 0, -60);
  plantLight2.target = housePlant2;
  plantLight2.position.set(housePlant2.position.x, 30, housePlant2.position.z);
  ...
  if (fbxCounter >= 4) {
    if (sup.checkCollisionForObject(human1.position, tree2.position)) {
      let bounds = sup.stopPlayer(human1.position, tree2.position);
  ...
  if (fbxCounter2 >= 4) {
    if (sup.checkCollisionForObject(human2.position, tree2.position)) {
      let bounds = sup.stopPlayer(human2.position, tree2.position);
  ...
  if (sup.checkCollisionForObject(football.position, tree2.position)) {
    let bounds = sup.stopPlayer(football.position, tree2.position);
...
window.addEventListener("keydown", (event) => {
  if (event.key == "z") {
    ...
    if (currentCamera == overHeadCamera1)
      console.log("current camera is up camera 1");
    else if (currentCamera == overHeadCamera2)
      console.log("current camera is up camera 2");
    if (currentCamera == overHeadCamera1) {
      firstPerson.lock();
    } else {
      firstPerson.unlock();
  ...
  if (event.key == " ") {
    ...
    if (ballPocession > 0) {
      ...
      ballDirection =
        ballPocession == 1
          ? human1Controls.runDirection
          : human2Controls.runDirection;
      ballPocession = 0;
    }
